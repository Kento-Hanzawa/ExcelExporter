...
#pragma warning restore RS0030 // 禁止された API を使用しない
    ...
    await Host.CreateDefaultBuilder()
        .ConfigureServices(static (HostBuilderContext context, IServiceCollection service) =>
        {
            ...
            var fileInfo = new FileInfo(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, ExcelExporter.Json.AppSettings.FileName));
            if (!fileInfo.Exists)
            {
                var settings = new Dictionary<string, object> { [context.HostingEnvironment.ApplicationName] = new ExcelExporter.Json.AppSettings(), };
            ...
            service.Configure<ExcelExporter.Json.AppSettings>(builder.Build().GetSection(context.HostingEnvironment.ApplicationName));
        })
...
#if DEBUG
        .RunConsoleAppFrameworkWebHostingAsync(ExcelExporter.Json.AppMain.SwaggerUrl);
#else
        .RunConsoleAppFrameworkAsync<ExcelExporter.Json.AppMain>(args);
#endif
}
finally
{
