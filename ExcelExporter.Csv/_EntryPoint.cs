...
#pragma warning restore RS0030 // 禁止された API を使用しない
    ...
    await Host.CreateDefaultBuilder()
        .ConfigureServices(static (HostBuilderContext context, IServiceCollection service) =>
        {
            ...
            var fileInfo = new FileInfo(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, ExcelExporter.Csv.AppSettings.FileName));
            if (!fileInfo.Exists)
            {
                var settings = new Dictionary<string, object> { [context.HostingEnvironment.ApplicationName] = new ExcelExporter.Csv.AppSettings(), };
            ...
            service.Configure<ExcelExporter.Csv.AppSettings>(builder.Build().GetSection(context.HostingEnvironment.ApplicationName));
        })
...
#if DEBUG
        .RunConsoleAppFrameworkWebHostingAsync(ExcelExporter.Csv.AppMain.SwaggerUrl);
#else
        .RunConsoleAppFrameworkAsync<ExcelExporter.Csv.AppMain>(args);
#endif
}
finally
{
